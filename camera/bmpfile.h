...
typedef struct  __attribute__((__packed__)) {	/* packed->구조체 패딩X */
    unsigned char bfType[2];                  	/* BM 표시 : "BM" (2 글자) 문자 */
    unsigned int bfSize;                      	/* 파일의 크기 : 4 바이트 정수  */
    unsigned short bfReserved1;    		/* 추후의 확장을 위한 필드(reserved) */
    unsigned short bfReserved2;    		/* 추후의 확장을 위한 필드(reserved) */
...
typedef struct {
    ...
    unsigned int biXPixelsPerMeter;      	/* 가로 해상도  : 4바이트 */
    unsigned int biYPixelsPerMeter;      	/* 세로 해상도 : 4바이트 */
